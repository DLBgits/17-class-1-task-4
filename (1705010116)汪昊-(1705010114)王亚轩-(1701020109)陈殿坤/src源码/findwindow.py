...
class FindWindow(tk.Toplevel):
    def __init__(self, master, **kwargs):
        ...
        self.all_replace_button = ttk.Button(bottom_frame, text="All_Replace", command=self.on_all_replace, style="editor.TButton")
        ...
        self.find_button.pack(side=tk.LEFT, padx=(0, 0))
        self.replace_button.pack(side=tk.LEFT, padx=(5, 5))
        self.all_replace_button.pack(side=tk.LEFT, padx=(5, 5))
    ...
    def on_find(self, not_all_find=False):
        return self.master.find(self.text_to_find.get(), not_all_find)

    def on_replace(self):
        self.master.replace_text(self.text_to_find.get(), self.text_to_replace_with.get())

#find全部符合要求的然后全部replace
    def on_all_replace(self):
        while(self.on_find(not_all_find=True)):
            self.on_replace()
